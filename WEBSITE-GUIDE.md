...
**Seeing the site's numbers.** Visitor tracking is now switched on — free,
privacy-friendly (no cookies, so it adds no cookie banner), and it costs
nothing at this traffic level. Steve can be added to the hosting account as a
**Viewer** to see it. That role is free too — Vercel does not charge for
viewer access — and it is read-only, so there is nothing he could
accidentally change or break. He would see the site, every update as it goes
out, how many people visited, which pages they looked at, and roughly how
fast each page loaded for them.
